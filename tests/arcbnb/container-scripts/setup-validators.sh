...
$BIN tx arcbnb set-orchestrator-address $($BIN keys show validator$i -a --bech val $ARGS) $ORCHESTRATOR_KEY $ETHEREUM_KEY --generate-only > /validator$i/add-arc-genesis-unsigned.json
...
TX=$(cat /validator$i/add-arc-genesis-signed.json)

jq ".app_state.genutil.gen_txs += [$TX]" /genesis.json > /edited-genesis.json && mv /edited-genesis.json /genesis.json

done

echo "Added arc transactions"
