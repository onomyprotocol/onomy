...
result=$( docker images -q onomy-arc-base )
...
set +e
docker rm -f onomy_arcbnb_all_up_test_instance
set -e
...
docker run --name onomy_arcbnb_all_up_test_instance $PLATFORM_CMD --cap-add=NET_ADMIN -t onomy-arc-base /bin/bash /onomy/tests/arcbnb/container-scripts/all-up-test-internal.sh $NODES $TEST_TYPE $ALCHEMY_ID
