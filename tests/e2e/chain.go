...
import (
	...
	metaprotocoltypes "github.com/onomyprotocol/onomy-rebuild/x/metaprotocols/types"
)
...
var (
	encodingConfig onomyparams.EncodingConfig
	cdc            codec.Codec
...
func newChain() (*chain, error) {
	tmpDir, err := os.MkdirTemp("", "onomy-e2e-testnet-")
	if err != nil {
		return nil, err
	}
...
func (c *chain) createValidator(index int) *validator {
	return &validator{
		chain:   c,
		index:   index,
		moniker: fmt.Sprintf("%s-onomy-%d", c.id, index),
	}
}
