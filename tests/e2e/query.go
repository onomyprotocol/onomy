...
func queryOnomyTx(endpoint, txHash string) error {
...
func getSpecificBalance(endpoint, addr, denom string) (amt sdk.Coin, err error) {
	balances, err := queryOnomyAllBalances(endpoint, addr)
	if err != nil {
		return amt, err
	}
...
func queryOnomyAllBalances(endpoint, addr string) (sdk.Coins, error) {
...
func queryGlobalFeeParams(endpoint string) (types.QueryParamsResponse, error) {
	body, err := httpGet(fmt.Sprintf("%s/onomy/globalfee/v1beta1/params", endpoint))
	if err != nil {
