...
const (
	...
	queryCommand   = "query"
	keysCommand    = "keys"
	onomyHomePath  = "/home/nonroot/.onomy"
	photonDenom    = "photon"
	uatomDenom     = "uatom"
	stakeDenom     = "stake"
...
var (
	onomyConfigPath   = filepath.Join(onomyHomePath, "config")
...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	//
	// 1. Initialize Onomy validator nodes.
	// 2. Create and initialize Onomy validator genesis files (both chains)
...
func (s *IntegrationTestSuite) runValidators(c *chain, portOffset int) {
	s.T().Logf("starting Onomy %s validator containers...", c.id)
	...
	for i, val := range c.validators {
		runOpts := &dockertest.RunOptions{
			...
			Mounts: []string{
				fmt.Sprintf("%s/:%s", val.configDir(), onomyHomePath),
			},
			Repository: "onomyprotocol/onomy-rebuildd-e2e",
		}
		...
		s.T().Logf("started Onomy %s validator container: %s", c.id, resource.Container.ID)
	}
	...
	s.Require().Eventually(
		...
		5*time.Minute,
		time.Second,
		"Onomy node failed to produce blocks",
	)
}
...
func (s *IntegrationTestSuite) runIBCRelayer() {
	...
	tmpDir, err := os.MkdirTemp("", "onomy-e2e-testnet-hermes-")
	s.Require().NoError(err)
	s.tmpDirs = append(s.tmpDirs, tmpDir)

	onomyAVal := s.chainA.validators[0]
	onomyBVal := s.chainB.validators[0]

	onomyARly := s.chainA.genesisAccounts[relayerAccountIndexHermes]
	onomyBRly := s.chainB.genesisAccounts[relayerAccountIndexHermes]
	...
	s.hermesResource, err = s.dkrPool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				...
				fmt.Sprintf("GAIA_A_E2E_VAL_MNEMONIC=%s", onomyAVal.mnemonic),
				fmt.Sprintf("GAIA_B_E2E_VAL_MNEMONIC=%s", onomyBVal.mnemonic),
				fmt.Sprintf("GAIA_A_E2E_RLY_MNEMONIC=%s", onomyARly.mnemonic),
				fmt.Sprintf("GAIA_B_E2E_RLY_MNEMONIC=%s", onomyBRly.mnemonic),
	...
	// create the client, connection and channel between the two Onomy chains
	s.createConnection()
	s.createChannel()
}
...
func configFile(filename string) string {
	filepath := filepath.Join(onomyConfigPath, filename)
	return filepath
}
