...
if ! test -f "./build/onomyd7"; then
  echo "onomyd v7 does not exist"
  exit
fi
...
cp ./build/onomyd7 "$NODE_HOME"/cosmovisor/genesis/bin/onomyd
...
cp ./build/onomyd8 "$NODE_HOME"/cosmovisor/upgrades/v8-Rho/bin/onomyd
