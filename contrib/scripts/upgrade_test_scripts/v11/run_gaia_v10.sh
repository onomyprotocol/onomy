...
if ! test -f "./build/onomyd10"; then
  echo "onomyd v10 does not exist"
  exit
fi
...
cp ./build/onomyd10 "$NODE_HOME"/cosmovisor/genesis/bin/onomyd
$BINARY init upgrader --chain-id $CHAINID --home "$NODE_HOME"


if ! test -f "./build/onomyd11"; then
  echo "onomyd v11 does not exist"
  exit
fi

mkdir -p "$NODE_HOME"/cosmovisor/upgrades/v11/bin
cp ./build/onomyd11 "$NODE_HOME"/cosmovisor/upgrades/v11/bin/onomyd
