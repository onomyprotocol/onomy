...
cp ./build/onomyd11 "$NODE_HOME"/cosmovisor/genesis/bin/onomyd
$BINARY init upgrader --chain-id $CHAINID --home "$NODE_HOME"


if ! test -f "./build/onomyd12"; then
  echo "onomyd v12 does not exist"
  exit
fi

mkdir -p "$NODE_HOME"/cosmovisor/upgrades/v12/bin
cp ./build/onomyd12 "$NODE_HOME"/cosmovisor/upgrades/v12/bin/onomyd
