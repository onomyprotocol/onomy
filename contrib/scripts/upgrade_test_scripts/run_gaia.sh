...
NODE_HOME=$(realpath ./build/.onomy)
echo "NODE_HOME = ${NODE_HOME}"
BINARY=$NODE_HOME/cosmovisor/genesis/bin/onomyd
echo "BINARY = ${BINARY}"
CHAINID=cosmoshub-4
...
if ! test -f "./build/onomydold"; then
  echo "old onomyd binary does not exist"
  exit
fi
...
cp ./build/onomydold "$NODE_HOME"/cosmovisor/genesis/bin/onomyd
$BINARY init upgrader --chain-id $CHAINID --home "$NODE_HOME"

if ! test -f "./build/onomydnew"; then
  echo "new onomyd binary does not exist"
  exit
fi

mkdir -p "$NODE_HOME"/cosmovisor/upgrades/$UPGRADE_VERSION/bin
cp ./build/onomydnew "$NODE_HOME"/cosmovisor/upgrades/$UPGRADE_VERSION/bin/onomyd
