...
if ! test -f "./build/onomyd8"; then
  echo "onomyd v8 does not exist"
  exit
fi
...
cp ./build/onomyd8 "$NODE_HOME"/cosmovisor/genesis/bin/onomyd
$BINARY init upgrader --chain-id $CHAINID --home "$NODE_HOME"


if ! test -f "./build/onomyd9"; then
  echo "onomyd v9 does not exist"
  exit
fi

mkdir -p "$NODE_HOME"/cosmovisor/upgrades/v9-lambda/bin
cp ./build/onomyd9 "$NODE_HOME"/cosmovisor/upgrades/v9-lambda/bin/onomyd
