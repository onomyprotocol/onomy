...
import (
	...
	"github.com/onomyprotocol/onomy-rebuild/x/globalfee/types"
)
...
func (mfd FeeDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
	...
	if !ok {
		return ctx, errorsmod.Wrap(onomyerrors.ErrTxDecode, "Tx must implement the sdk.FeeTx interface")
	}
	...
	if feeTx.GetFee().Len() > feeRequired.Len() {
		return ctx, errorsmod.Wrapf(onomyerrors.ErrInvalidCoins, "fee is not a subset of required fees; got %s, required: %s", feeTx.GetFee().String(), feeRequired.String())
	}
	...
	if !feeCoinsNonZeroDenom.DenomsSubsetOf(nonZeroCoinFeesReq) {
		return ctx, errorsmod.Wrapf(onomyerrors.ErrInsufficientFee, "fee is not a subset of required fees; got %s, required: %s", feeCoins.String(), feeRequired.String())
	}
	...
	if len(feeCoins) == 0 {
		...
		return ctx, errorsmod.Wrapf(onomyerrors.ErrInsufficientFee, "insufficient fees; got: %s required: %s", feeCoins.String(), feeRequired.String())
	}
	...
	if !feeCoinsNonZeroDenom.IsAnyGTE(nonZeroCoinFeesReq) {
		...
		return ctx, errorsmod.Wrap(onomyerrors.ErrInsufficientFee, errMsg)
	}
