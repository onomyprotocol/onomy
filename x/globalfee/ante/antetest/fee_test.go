...
func (s *IntegrationTestSuite) TestGlobalFeeMinimumGasFeeAnteHandler() {
	...
	testCases := map[string]struct {
		...
		// cases from https://github.com/onomyprotocol/onomy-rebuild/pull/1570#issuecomment-1190524402
...
//
// Note that in a real Onomy deployment all zero coins can be removed from minGasPrice.
// This sanitizing happens when the minGasPrice is set into the context.
// (see baseapp.SetMinGasPrices in onomy/cmd/root.go line 221)
func (s *IntegrationTestSuite) TestGetMinGasPrice() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			fees := onomyfeeante.GetMinGasPrice(s.ctx, int64(tc.feeTxGasLimit))
