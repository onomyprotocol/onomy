...
var (
	...
	// see DefaultZeroGlobalFee method in onomy/x/globalfee/ante/fee.go.
	...
	// For details, see onomyfeeante.NewFeeDecorator()
...
func validateMinimumGasPrices(i interface{}) error {
	v, ok := i.(sdk.DecCoins)
	if !ok {
		return errorsmod.Wrapf(onomyerrors.ErrInvalidType, "type: %T, expected sdk.DecCoins", i)
	}
...
func validateBypassMinFeeMsgTypes(i interface{}) error {
	...
	if !ok {
		return errorsmod.Wrapf(onomyerrors.ErrInvalidType, "type: %T, expected []sdk.Msg", i)
	}
...
func validateMaxTotalBypassMinFeeMsgGasUsage(i interface{}) error {
	_, ok := i.(uint64)
	if !ok {
		return errorsmod.Wrapf(onomyerrors.ErrInvalidType, "type: %T, expected uint64", i)
	}
