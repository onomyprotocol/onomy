...
- **Tendermint/CometBFT key**: A unique key that is used to sign consensus votes.
  - It is associated with a public key `cosmosvalconspub` (To get this value, run `onomyd tendermint show-validator`)
  - It is generated when the node is created with `onomyd init`.
- **Application key**: This key is created from the `onomyd` binary and is used to sign transactions. Application keys are associated with a public key that is prefixed by `cosmospub` and an address that is prefixed by `cosmos`.

The Tendermint/CometBFT key and the application key are derived from account keys that are generated by the `onomyd keys add` command.
...
```
onomyd tx staking validator-bond cosmosvaloper13h5xdxhsdaugwdrkusf8lkgu406h8t62jkqv3h <delegator> --from mykey  
```
