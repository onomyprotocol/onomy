...
:::tip
### **v14 Upgrade**
Cosmos Hub will be upgraded to [v14.1](https://github.com/onomyprotocol/onomy-rebuild/releases/tag/v14.1.0) at block height: **[18,262,000](https://www.mintscan.io/cosmos/blocks/18262000)**
...
`onomyd` is a command-line interface that lets you interact with the Cosmos Hub. `onomyd` is the only tool that supports 100% of the Cosmos Hub features, including accounts, transfers, delegation, and governance. Learn more about `onomyd` with the [delegator's CLI guide](/delegators/delegator-guide-cli.md).

## Running a full-node on the Cosmos Hub Mainnet

In order to run a full-node for the Cosmos Hub mainnet, you must first [install `onomyd`](/getting-started/installation.md). Then, follow [the guide](/hub-tutorials/join-mainnet.md).
