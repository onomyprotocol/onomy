...
- You will need to compile [`onomyd`](https://hub.cosmos.network/main/getting-started/installation.html) from source into a binary file executable by your operating system eg. MacOS, Windows, Linux
...
For information on how to use onomyd (the command line interface) to submit an on-chain proposal through the governance module, please refer to the [onomyd CLI tutorials](../hub-tutorials/onomyd.mdx) for the Cosmos Hub documentation.
...
These proposals can be submitted using `onomyd tx gov submit-legacy-proposal`.
...
These proposals can are submitted using `onomyd tx gov submit-proposal`.

Using `onomyd tx gov draft-proposal` can help prepare a proposal. The tool will create a file containing the specified proposal message and it also helps with populating all the required proposal fields.
...
```shell
onomyd q staking params -o json
{
...
Upon completion, the proposal will be available in the directory where you called the `onomyd` command inside the `draft_proposal.json` file.
...
```sh
onomyd tx gov submit-proposal <path_to_proposal.json>
...
Use `onomyd tx gov --help` to get more info about the CLI options, we will explain some options below:
...
```sh
onomyd q gov proposals --depositor cosmos1hxv7mpztvln45eghez6evw2ypcw4vjmsmr8cdx
```

Once you have the proposal ID, this is the command to deposit extra tokens:

```sh
onomyd tx gov deposit <proposal-id> <deposit_amount> --from <name>
```
