...
------------
If passed, this governance proposal would commit the Cosmos Hub to halting the `cosmoshub-3` at 06:00 UTC on Feb 18th exporting the state and starting `cosmoshub-4` based on onomy 4.0.0
...
* Bugs have been identified in the Proposal 29 implementation.  They are resolved in this pull request[Additional review of prop 29 and migration testing by zmanian · Pull Request #559 · onomyprotocol/onomy-rebuild · GitHub](https://github.com/onomyprotocol/onomy-rebuild/pull/559)
...
This upgrade also implements the fund recovery procedure defined in proposal 29. The code is [here](https://github.com/onomyprotocol/onomy-rebuild/blob/main/app/prop29.go). As requested during the discussion of the governance proposal, the migration and fund recovery procedure verifies all signatures. This code was tested during the cosmoshub-3 simulated upgrade testnet.
...
The upgrade instructions will continue to improve as a living [document](https://github.com/onomyprotocol/onomy-rebuild/blob/main/docs/migration/cosmoshub-3.md). The Onomy maintainers will continue to improve the instruction below.

This section is with the current `onomy 2.0.*` implementation.

  1. Validators should set their `onomy` with a halt time of `1613628000`ie. 06:00 UTC on Feb 18th in Unix time.

  2. Validators should then export the current cosmos state with `onomyd export > cosmoshub-3-export.json`

  3. Validators should determine the height of the last block as `last-cosmoshub-3_block`. This can be easily determined from onomy logs.

  4. Validators should back up their `.onomyd` directory.

This section is with the upgraded `onomy 4.0.*` implemenataion.

  1. Validators should then migrate the exported genesis state. `onomyd migrate cosmoshub-3-export.json --chain-id=cosmoshub-4 --initial-height [last_cosmoshub-3_block+1] > genesis.json`
  2. Validators should (**AFTER ENSURING IT IS BACKED UP**!) delete their `~/.onomyd` directory and create a new one with `onomyd init [name]` and then edit their configuration files or recover the config files from backups.
  3. Validators should then start `cosmoshub-4` with `onomyd start`. Automated audits of the genesis state can take 30-120 min using the crisis module. This can be disabled by `onomyd start --x-crisis-skip-assert-invariants`.
...
## The git commit of Onomy that we are upgrading to
...
We will upgrade the stargate-final testnet to support Onomy v4.0 and continue the integration progress with exchanges, blockexplorers and wallets.
...
1. A critical vulnerability may be found in the software. If the development teams change their recommended version of onomy, the validator set should implicitly abandon this upgrade procedure. A signaling proposal will be made to indicate a vulnerability or defect in the upgrade has been found. A future proposal will be made to the Hub to upgrade to the new target commit.
