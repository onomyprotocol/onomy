...
There are ways to query the current settings for each module's parameter(s). Some can be queried with the command line program [`onomyd`](../../getting-started/installation.md).

You can begin by using the command `onomy q [module] -h` to get help about the subcommands for the module you want to query. For example, `onomyd q staking params --chain-id <chain-id> --node <node-id>` returns the settings of relevant parameters:
