...
echo "export DAEMON_HOME=$HOME/.onomy" >> ~/.profile
source ~/.profile
```
...
```bash
mkdir -p ~/.onomy/cosmovisor/upgrades
mkdir -p ~/.onomy/cosmovisor/genesis/bin/
cp $(which onomyd) ~/.onomy/cosmovisor/genesis/bin/

# verify the setup. 
# It should return the same version as onomyd
cosmovisor version
```

Now `onomyd` can start by running
...
First, stop your instance of `onomyd`. Next, upgrade the software:
...
See the [testnet repo](https://github.com/cosmos/testnets) for details on which version is needed for which public testnet, and the [Onomy release page](https://github.com/cosmos/Onomy/releases) for details on each release.
...
```bash
cd $HOME/.onomy/config
onomyd export --for-zero-height --height=<export-height> > new_genesis.json
```
...
:::warning
If you are running a **validator node** on the mainnet, always be careful when doing `onomyd unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
