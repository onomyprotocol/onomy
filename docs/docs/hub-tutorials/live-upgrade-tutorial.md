...
1. Start the network and trigger upgrade
   ...
   ```bash
   # start a onomy application full-node
   $ onomyd start

   # set up the cli config
   $ onomyd config chain-id testing

   # create an upgrade governance proposal
   $ onomyd tx gov submit-proposal software-upgrade <plan-name> \
...
2. Performing an upgrade
   ...
   manually kill the process and replace it with a new binary. Do so now with `Ctrl+C` or `killall onomyd`.

   In `onomy/app/app.go`, after `upgrade.Keeper` is initialized and set in the app, set the
   ...
   ```bash
   # create a new binary of onomy with the added upgrade handler
   $ make install
   ...
   $ onomyd query upgrade applied <plan-name>
   ```
