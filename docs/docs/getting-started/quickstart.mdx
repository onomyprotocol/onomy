...
> **Note**: Make sure the [Onomy CLI is installed](./installation.md).
...
```bash
# Build onomyd binary and initialize chain
cd $HOME
git clone -b v15.0.0 https://github.com/onomyprotocol/onomy-rebuild --depth=1
cd onomyd
make install
onomyd init CUSTOM_MONIKER --chain-id cosmoshub-4

#Set minimum gas price & peers
sed -i'' 's/minimum-gas-prices = ""/minimum-gas-prices = "0.0025uatom"/' $HOME/.onomy/config/app.toml
sed -i'' 's/persistent_peers = ""/persistent_peers = '"\"$(curl -s https://raw.githubusercontent.com/cosmos/chain-registry/master/cosmoshub/chain.json | jq -r '[foreach .peers.seeds[] as $item (""; "\($item.id)@\($item.address)")] | join(",")')\""'/' $HOME/.onomy/config/config.toml

# Configure State sync
sed -i'' 's/enable = false/enable = true/' $HOME/.onomy/config/config.toml
sed -i'' 's/trust_height = 0/trust_height = <BLOCK_HEIGHT>/' $HOME/.onomy/config/config.toml
sed -i'' 's/trust_hash = ""/trust_hash = "<BLOCK_HASH>"/' $HOME/.onomy/config/config.toml
sed -i'' 's/rpc_servers = ""/rpc_servers = "https:\/\/cosmos-rpc.polkachu.com:443,https:\/\/rpc-cosmoshub-ia.cosmosia.notional.ventures:443,https:\/\/rpc.cosmos.network:443"/' $HOME/.onomy/config/config.toml
...
**Note**: Make sure to set the `--home` flag when initializing and starting `onomyd` if mounting quicksync data externally.

##### Create Onomy Home & Config

```bash
mkdir $HOME/.onomy/config -p
```
...
```bash
curl https://quicksync.io/addrbook.cosmos.json > $HOME/.onomy/config/addrbook.json
```
