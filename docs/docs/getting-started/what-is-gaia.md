...
`onomy` is the name of the Cosmos SDK application for the Cosmos Hub. It comes with 2 main entrypoints:

- `onomyd`: The Onomy Daemon and command-line interface (CLI). runs a full-node of the `onomy` application.

`onomy` is built on the Cosmos SDK using the following modules:
...
Next, learn how to [install Onomy](./installation.md).
