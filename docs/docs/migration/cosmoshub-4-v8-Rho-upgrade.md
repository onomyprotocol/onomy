...
- [globalfee](https://github.com/onomyprotocol/onomy-rebuild/tree/main/x/globalfee) module. See [globalfee docs](https://github.com/onomyprotocol/onomy-rebuild/blob/main/docs/modules/globalfee.md) for more details.
- [#1845](https://github.com/onomyprotocol/onomy-rebuild/pull/1845) Add bech32-convert command to onomyd.
- [Add new fee decorator](https://github.com/onomyprotocol/onomy-rebuild/pull/1961) to change `MaxBypassMinFeeMsgGasUsage` so importers of x/globalfee can change `MaxGas`.
- [#1870](https://github.com/onomyprotocol/onomy-rebuild/issues/1870) Fix bank denom metadata in migration. See [#1892](https://github.com/onomyprotocol/onomy-rebuild/pull/1892) for more details.
- [#1976](https://github.com/onomyprotocol/onomy-rebuild/pull/1976) Fix Quicksilver ICA exploit in migration. See [the bug fix forum post](https://forum.cosmos.network/t/upcoming-interchain-accounts-bugfix-release/8911) for more details.
- [E2E tests](https://github.com/onomyprotocol/onomy-rebuild/tree/main/tests/e2e). The tests cover transactions/queries tests of different modules, including Bank, Distribution, Encode, Evidence, FeeGrant, Global Fee, Gov, IBC, packet forwarding middleware, Slashing, Staking, and Vesting module.
- [#1941](https://github.com/onomyprotocol/onomy-rebuild/pull/1941) Fix packet forward configuration for e2e tests.
- Use onomyd to swap out [Ignite](https://github.com/ignite/cli) in [liveness tests](https://github.com/onomyprotocol/onomy-rebuild/blob/main/.github/workflows/test.yml).
...
- [Cosmos Hub 4, v8-Rho Upgrade, Instructions](#cosmos-hub-4-v8-rho-upgrade-instructions)
  ...
  - [Preparing for the upgrade](#preparing-for-the-upgrade)
    ...
    - [Backups](#backups)
    - [Testing](#testing)
    - [Current runtime, cosmoshub-4 (pre-v8-Rho upgrade) is running Onomy v7.0.x](#current-runtime-cosmoshub-4-pre-v7-theta-upgrade-is-running-onomy-v60x)
    - [Target runtime, cosmoshub-4 (post-v8-Rho upgrade) will run Onomy v8.0.0](#target-runtime-cosmoshub-4-post-v8-rho-upgrade-will-run-onomy-v800)
  - [v8-Rho upgrade steps](#v8-Rho-upgrade-steps)
    - [Method I: Manual Upgrade](#method-i-manual-upgrade)
    - [Method II: Upgrade using Cosmovisor](#method-ii-upgrade-using-cosmovisor)
      - [Manually preparing the binary](#manually-preparing-the-onomy-v800-binary)
        - [Preparation](#preparation)
        - [Expected upgrade result](#expected-upgrade-result)
      - [Auto-Downloading the Onomy v8.0.0 binary (not recommended!)](#auto-downloading-the-onomy-v800-binary-not-recommended)
...
### Current runtime, cosmoshub-4 (pre-v8-Rho upgrade) is running Onomy v7.1.1

The Cosmos Hub mainnet network, `cosmoshub-4`, is currently running [Onomy v7.1.1](https://github.com/onomyprotocol/onomy-rebuild/releases/v7.1.1). We anticipate that operators who are running on v7.1.1, will be able to upgrade successfully; however, this is untested and it is up to operators to ensure that their systems are capable of performing the upgrade.

### Target runtime, cosmoshub-4 (post-v8-Rho upgrade) will run Onomy v8.0.0

The Cosmos Hub mainnet network, `cosmoshub-4`, will run [Onomy v8.0.0](https://github.com/onomyprotocol/onomy-rebuild/releases/tag/v8.0.0). Operators _MUST_ use this version post-upgrade to remain connected to the network.
...
Run Onomy v7.1.1 till upgrade height, the node will panic:

```shell
ERR UPGRADE "v8-Rho" NEEDED at height: 14099412: upgrade to v7-Theta and applying upgrade "v8-Rho" at height:14099412
```

Stop the node, and install Onomy v8.0.0 and re-start by `onomyd start`.
...
>
> **For Example:** <br/>
> **Cosmovisor =< `1.1.0`: `/upgrades/v8-Rho/bin/onomyd`**<br/>
> **Cosmovisor >= `1.2.0`: `/upgrades/v8-rho/bin/onomyd`**<br/>
...
### _Manually preparing the Onomy v8.0.0 binary_
...
create a Cosmovisor folder inside `$GAIA_HOME` and move Onomy v7.1.1 into `$GAIA_HOME/cosmovisor/genesis/bin`
...
build Onomy v8.0.0, and move onomyd v8.0.0 to `$GAIA_HOME/cosmovisor/upgrades/v8-rho/bin`

```shell
mkdir -p  $GAIA_HOME/cosmovisor/upgrades/v8-rho/bin
cp $(which onomyd) $GAIA_HOME/cosmovisor/upgrades/v8-rho/bin
```
...
### _Auto-Downloading the Onomy v8.0.0 binary (not recommended!)_
#### Preparation
...
create a cosmovisor folder inside onomy home and move onomyd v7.1.1 into `$GAIA_HOME/cosmovisor/genesis/bin`
...
Then the Cosmovisor will create `$GAIA_HOME/cosmovisor/upgrades/v8-rho/bin` and download the Onomy v8.0.0 binary to this folder according to links in the `--info` field of the upgrade proposal 97.
...
> onomyd start --unsafe-skip-upgrade 14099412
