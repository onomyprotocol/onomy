...
This document describes the steps for validator and full node operators for the successful execution of the [v7-Theta Upgrade](https://github.com/onomyprotocol/onomy-rebuild/blob/main/docs/roadmap/cosmos-hub-roadmap-2.0.md#v7-theta-upgrade-expected-q1-2022), which contains the following main new features/improvement:
...
- [Cosmos Hub 4, v7-Theta Upgrade, Instructions](#cosmos-hub-4-v7-theta-upgrade-instructions)
  ...
  - [Preparing for the upgrade](#preparing-for-the-upgrade)
    ...
    - [Backups](#backups)
    - [Testing](#testing)
    - [Current runtime, cosmoshub-4 (pre-v7-Theta upgrade) is running Onomy v6.0.x](#current-runtime-cosmoshub-4-pre-v7-theta-upgrade-is-running-onomy-v60x)
    - [Target runtime, cosmoshub-4 (post-v7-Theta upgrade) will run Onomy v7.0.0](#target-runtime-cosmoshub-4-post-v7-theta-upgrade-will-run-onomy-v700)
  - [v7-Theta upgrade steps](#v7-theta-upgrade-steps)
    - [Method I: manual upgrade](#method-i-manual-upgrade)
    - [Method II: upgrade using Cosmovisor by manually preparing the Onomy v7.0.0 binary](#method-ii-upgrade-using-cosmovisor-by-manually-preparing-the-onomy-v700-binary)
      - [Preparation](#preparation)
      - [Expected upgrade result](#expected-upgrade-result)
    - [Method III: upgrade using Cosmovisor by auto-downloading the Onomy v7.0.0 binary (not recommended!)](#method-iii-upgrade-using-cosmovisor-by-auto-downloading-the-onomy-v700-binary-not-recommended)
...
### Current runtime, cosmoshub-4 (pre-v7-Theta upgrade) is running Onomy v6.0.x

The Cosmos Hub mainnet network, `cosmoshub-4`, is currently running [Onomy v6.0.4](https://github.com/onomyprotocol/onomy-rebuild/releases/v6.0.4). We anticipate that operators who are running on v6.0.x, will be able to upgrade successfully; however, this is untested and it is up to operators to ensure that their systems are capable of performing the upgrade.

### Target runtime, cosmoshub-4 (post-v7-Theta upgrade) will run Onomy v7.0.0

The Cosmos Hub mainnet network, `cosmoshub-4`, will run [Onomy v7.0.0](https://github.com/onomyprotocol/onomy-rebuild/releases/tag/v7.0.0). Operators _MUST_ use this version post-upgrade to remain connected to the network.
...
Run Onomy v6.0.x till upgrade height, the node will panic:
...
Stop the node, and install Onomy v7.0.0 and re-start by `onomyd start`.
...
### Method II: upgrade using Cosmovisor by manually preparing the Onomy v7.0.0 binary
...
create a Cosmovisor folder inside `$GAIA_HOME` and move Onomy v6.0.4 into `$GAIA_HOME/cosmovisor/genesis/bin`
...
build Onomy v7.0.0, and move onomyd v7.0.0 to `$GAIA_HOME/cosmovisor/upgrades/v7-Theta/bin`

```shell
mkdir -p  $GAIA_HOME/cosmovisor/upgrades/v7-Theta/bin
cp $(which onomyd) $GAIA_HOME/cosmovisor/upgrades/v7-Theta/bin
```
...
### Method III: upgrade using Cosmovisor by auto-downloading the Onomy v7.0.0 binary (not recommended!)
...
create a cosmovisor folder inside onomy home and move onomyd v6.0.4 into `$GAIA_HOME/cosmovisor/genesis/bin`
...
Then the Cosmovisor will create `$GAIA_HOME/cosmovisor/upgrades/v7-Theta/bin` and download Onomy v7.0.0 binary to this folder according to links in the `--info` field of the upgrade proposal 65.
...
> onomyd start --unsafe-skip-upgrade 10085397
