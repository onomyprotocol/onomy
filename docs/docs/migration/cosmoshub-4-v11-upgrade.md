---
title: Cosmos Hub 4, Onomy v11 Upgrade Instructions
order: 7
---
<!-- markdown-link-check-disable -->

This document describes the steps for validators and full node operators, to upgrade successfully to the Onomy v11 release.
For more details on the release, please see the [release notes](https://github.com/onomyprotocol/onomy-rebuild/releases/tag/v11.0.0)

## Instructions
  
- [Cosmos Hub 4, Onomy v11 Upgrade, Instructions](#cosmos-hub-4-onomy-v11-upgrade-instructions)
...
The Cosmos Hub mainnet network, `cosmoshub-4`, is currently running [Onomy v10.0.2](https://github.com/onomyprotocol/onomy-rebuild/releases/v10.0.2). We anticipate that operators who are running on v10.0.x, will be able to upgrade successfully. Validators are expected to ensure that their systems are up to date and capable of performing the upgrade. This includes running the correct binary, or if building from source, building with go `1.20`.

### Target runtime

The Cosmos Hub mainnet network, `cosmoshub-4`, will run [Onomy v11.0.0](https://github.com/onomyprotocol/onomy-rebuild/releases/tag/v11.0.0). Operators _**MUST**_ use this version post-upgrade to remain connected to the network.
...
Make sure Onomy v11.0.0 is installed by either downloading a [compatible binary](https://github.com/onomyprotocol/onomy-rebuild/releases/tag/v11.0.0), or building from source. Building from source requires **Golang 1.20**.

Run Onomy v10.0.x till upgrade height, the node will panic:

```shell
ERR UPGRADE "v11" NEEDED at height: 16596000: upgrade to v11 and applying upgrade "v11" at height:16596000
```

Stop the node, and switch the binary to Onomy v11.0.0 and re-start by `onomyd start`.
...
build Onomy v11.0.0, and move onomyd v11.0.0 to `$GAIA_HOME/cosmovisor/upgrades/v11/bin`

```shell
mkdir -p  $GAIA_HOME/cosmovisor/upgrades/v11/bin
cp $(which onomyd) $GAIA_HOME/cosmovisor/upgrades/v11/bin
```
...
create a cosmovisor folder inside onomy home and move onomyd v10.0.x into `$GAIA_HOME/cosmovisor/genesis/bin`
...
Then the Cosmovisor will create `$GAIA_HOME/cosmovisor/upgrades/v11/bin` and download the Onomy v11.0.0 binary to this folder according to links in the `--info` field of the upgrade proposal.
...
> onomyd start --unsafe-skip-upgrade 16596000
