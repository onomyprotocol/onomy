...
[Onomy](https://github.com/onomyprotocol/onomy-rebuild) application have been split into separate
repositories. This allows for both the Cosmos SDK and Onomy to evolve naturally
and independently. Thus, any future [releases](https://github.com/onomyprotocol/onomy-rebuild/releases)
of Onomy going forward, including this one, will be built and tagged from this
repository not the Cosmos SDK.

Since the Cosmos SDK and Onomy have now been split into separate repositories, their
...
on its current versioning path (i.e. v0.36.x ) and the [Onomy](https://github.com/onomyprotocol/onomy-rebuild)
application will become v2.0.x.

__[Onomy](https://github.com/onomyprotocol/onomy-rebuild) application v2.0.3 is
...
can be done by backing up the `.onomy` directories.
...
- The version/commit hash of Onomy v2.0.3: `2f6783e298f25ff4e12cb84549777053ab88749a`
...
- You may obtain the canonical UTC timestamp of the exported block by any of the following methods:
  ...
  - Through manually querying a Onomy REST client (e.g. `/blocks/2902000`)
...
2. Export existing state from `cosmoshub-2`:
   ...
   ```bash
   onomyd export --for-zero-height --height=2902000 > cosmoshub_2_genesis_export.json
   ```
...
v2.0.3 of [Onomy](https://github.com/onomyprotocol/onomy-rebuild).

   __NOTE__: Go [1.13+](https://golang.org/dl/) is required!

   ```bash
   git clone https://github.com/onomyprotocol/onomy-rebuild.git && cd onomy && git checkout v2.0.3; make install
   ```

1. Verify you are currently running the correct version (v2.0.3) of the _Onomy_:
...
2. Migrate exported state from the current v0.34.6+ version to the new v2.0.3 version:
   ...
   ```bash
   onomyd migrate v0.36 cosmoshub_2_genesis_export.json --chain-id=cosmoshub-3 --genesis-time=[PLACEHOLDER]> genesis.json
   ```
...
3.  Move the new `genesis.json` to your `.onomy/config/` directory
4.  Replace the `db_backend` on `.onomy/config/config.toml` to:

    ```toml
    db_backend = "goleveldb"
    ```

5.  Note, if you have any application configuration in `onomyd.toml`, that file has now been renamed to `app.toml`:

    ```bash
    mv .onomy/config/onomyd.toml .onomy/config/app.toml
    ```
...
4. We highly recommend standing up a [testnet](https://github.com/onomyprotocol/onomy-rebuild/blob/master/docs/deploy-testnet.md)
   with the `onomy-2.0` release or joining the onomy-13006 testnet. More info for joining the testnet can be
