...
- [Service Providers](#service-providers)
  ...
  - [Command-Line Interface](#command-line-interface)
    ...
    - [Remote Access to onomyd](#remote-access-to-onomyd)
...
The command-line interface (CLI) is the most powerful tool to access the Cosmos Hub and use onomy.
To use the CLI, you must install the latest version of `onomy` on your machine.

Compare your version with the [latest release version](https://github.com/onomyprotocol/onomy-rebuild/releases)
...
All available CLI commands are shown when you run the `onomyd` command:
...
```bash
Onomy Internet Financial System
...
Flags:
  -h, --help                help for onomyd
      --home string         directory for config and data (default "/Users/tobias/.onomy")
      --log_format string   The logging format (json|plain) (default "plain")
      --log_level string    The logging level (trace|debug|info|warn|error|fatal|panic) (default "info")
      --trace               print out full stack trace on errors

Use "onomyd [command] --help" for more information about a command.
```
...
Use "onomyd query [command] --help" for more information about a command.
```

### Remote Access to onomyd

When choosing to remote access a Full Node and onomyd, you need a Full Node running and onomy installed on your local machine.

`onomyd` is the tool that enables you to interact with the node that runs on the Cosmos Hub network, whether you run it yourself or not.

To set up `onomyd` on a local machine and connect to an existing full node, use the following command:
...
The default key is `secp256k1 elliptic curve`. Use the `onomyd keys` command to list the keys and generate a new key.

```bash
onomyd keys add <your_key_name>
```
...
```bash
onomyd keys add <your_key_name> --recover
```
...
```bash
onomyd query account <YOUR_ADDRESS>
```
...
```bash
onomyd query bank balances <YOUR_ADDRESS>
```
