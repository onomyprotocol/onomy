...
import (
	...
	"github.com/onomyprotocol/onomy-rebuild/app/helpers"
	v15 "github.com/onomyprotocol/onomy-rebuild/app/upgrades/v15"
)

func TestUpgradeSigningInfos(t *testing.T) {
	onomyApp := helpers.Setup(t)
	ctx := onomyApp.NewUncachedContext(true, tmproto.Header{})
	slashingKeeper := onomyApp.SlashingKeeper
...
func TestUpgradeMinCommissionRate(t *testing.T) {
	...
	stakingParams := onomyApp.StakingKeeper.GetParams(ctx)
	stakingParams.MinCommissionRate = sdk.ZeroDec()
	err := onomyApp.StakingKeeper.SetParams(ctx, stakingParams)
	require.NoError(t, err)

	stakingKeeper := onomyApp.StakingKeeper
...
func TestClawbackVestingFunds(t *testing.T) {
	...
	bankKeeper := onomyApp.BankKeeper
	accountKeeper := onomyApp.AccountKeeper
	distrKeeper := onomyApp.DistrKeeper
	stakingKeeper := onomyApp.StakingKeeper

	ctx := onomyApp.NewUncachedContext(true, tmproto.Header{Height: 1})
	...
	require.NoError(t, v15.ClawbackVestingFunds(ctx.WithBlockTime(endTime), addr, &onomyApp.AppKeepers))
	...
	require.NoError(t, v15.ClawbackVestingFunds(ctx, addr, &onomyApp.AppKeepers))
	...
	onomyApp.EndBlock(abci.RequestEndBlock{})
	onomyApp.BeginBlock(
...
func TestSetMinInitialDepositRatio(t *testing.T) {
	...
	err := v15.SetMinInitialDepositRatio(ctx, *onomyApp.GovKeeper)
	require.NoError(t, err)

	minInitialDepositRatioStr := onomyApp.GovKeeper.GetParams(ctx).MinInitialDepositRatio
...
func TestUpgradeEscrowAccounts(t *testing.T) {
	...
	bankKeeper := onomyApp.BankKeeper
	transferKeeper := onomyApp.TransferKeeper
