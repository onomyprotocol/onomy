...
TEST_DOCKER_REPO=cosmos/contrib-onomytest
...
ifneq ($(GO_SYSTEM_VERSION), $(REQUIRE_GO_VERSION))
	@echo "ERROR: Go version 1.21 is required for $(VERSION) of Onomy."
endif
...
draw-deps:
	...
	@goviz -i ./cmd/onomyd -d 2 | dot -Tpng -o dependency-graph.png
...
docker-build-debug:
	@docker build -t onomyprotocol/onomy-rebuildd-e2e -f e2e.Dockerfile .
...
start-localnet-ci: build
	rm -rf ~/.onomyd-liveness
	./build/onomyd init liveness --chain-id liveness --home ~/.onomyd-liveness
	./build/onomyd config chain-id liveness --home ~/.onomyd-liveness
	./build/onomyd config keyring-backend test --home ~/.onomyd-liveness
	./build/onomyd keys add val --home ~/.onomyd-liveness
	./build/onomyd genesis add-genesis-account val 10000000000000000000000000stake --home ~/.onomyd-liveness --keyring-backend test
	./build/onomyd genesis gentx val 1000000000stake --home ~/.onomyd-liveness --chain-id liveness
	./build/onomyd genesis collect-gentxs --home ~/.onomyd-liveness
	sed -i.bak'' 's/minimum-gas-prices = ""/minimum-gas-prices = "0uatom"/' ~/.onomyd-liveness/config/app.toml
	./build/onomyd start --home ~/.onomyd-liveness --x-crisis-skip-assert-invariants
