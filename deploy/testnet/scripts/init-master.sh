...
onomyd keys add --output=json $ONOMY_ORCHESTRATOR_NAME $ONOMY_KEYRING_FLAG | jq . >> $ONOMY_HOME/orchestrator_key.json
...
onomyd add-genesis-account "$(onomyd keys show $ONOMY_VALIDATOR_NAME -a $ONOMY_KEYRING_FLAG)" $ONOMY_GENESIS_COINS
echo "Adding orchestrator addresses to genesis files"
onomyd add-genesis-account "$(onomyd keys show $ONOMY_ORCHESTRATOR_NAME -a $ONOMY_KEYRING_FLAG)" $ONOMY_GENESIS_COINS
...
do
	...
	onomyd keys add --output=json account$i $ONOMY_KEYRING_FLAG | jq . >> $ONOMY_HOME/account$i.json
	onomyd add-genesis-account "$(onomyd keys show account$i -a $ONOMY_KEYRING_FLAG)" $ONOMY_GENESIS_COINS
done

echo "Creating gentxs"
onomyd gentx --ip $ONOMY_HOST $ONOMY_VALIDATOR_NAME $ONOMY_STAKE_COINS $ETH_ORCHESTRATOR_VALIDATOR_ADDRESS "$(jq -r .address $ONOMY_HOME/orchestrator_key.json)" $ONOMY_KEYRING_FLAG $ONOMY_CHAINID_FLAG
