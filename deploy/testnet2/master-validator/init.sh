...
ONOMY_GENESIS_COINS="10000000000000000$STAKE_DENOM,100000000000000$NORMAL_DENOM"
...
jq '.app_state.gov.voting_params.voting_period = "172800s"' $CURRENT_WORKING_DIR/metadata-genesis.json > $CURRENT_WORKING_DIR/edited-genesis.json
jq '.consensus_params.evidence.max_age_duration = "259200000000000"' $CURRENT_WORKING_DIR/metadata-genesis.json > $CURRENT_WORKING_DIR/edited-genesis.json
jq '.app_state.staking.params.unbonding_time = "259200s"' $CURRENT_WORKING_DIR/metadata-genesis.json > $CURRENT_WORKING_DIR/edited-genesis.json
jq '.app_state.mint.params.blocks_per_year = "5006000"' $CURRENT_WORKING_DIR/metadata-genesis.json > $CURRENT_WORKING_DIR/edited-genesis.json
jq '.app_state.slashing.params.signed_blocks_window = "7200"' $CURRENT_WORKING_DIR/metadata-genesis.json > $CURRENT_WORKING_DIR/edited-genesis.json
jq '.app_state.slashing.params.min_signed_per_window = "0.050000000000000000"' $CURRENT_WORKING_DIR/metadata-genesis.json > $CURRENT_WORKING_DIR/edited-genesis.json
...
$ONOMY gentx --ip $ONOMY_HOST $ONOMY_VALIDATOR_NAME 1000000000$STAKE_DENOM "$(jq -r .address $ONOMY_HOME/eth_key.json)" "$(jq -r .address $ONOMY_HOME/orchestrator_key.json)" $ONOMY_KEYRING_FLAG $ONOMY_CHAINID_FLAG
