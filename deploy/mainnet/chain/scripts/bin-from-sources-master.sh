...
cd $ETH_BRIDGE_SRC/solidity
npm ci
chmod -R +x scripts
npm run typechain
npm run compile-deployer

cp $ETH_BRIDGE_SRC/solidity/contract-deployer $ONOMY_HOME/bin/contract-deployer

mkdir -p $ONOMY_HOME/contracts/eth-bridge
cp $ETH_BRIDGE_SRC/solidity/artifacts/contracts/Gravity.sol/Gravity.json $ONOMY_HOME/contracts/eth-bridge/Gravity.json
