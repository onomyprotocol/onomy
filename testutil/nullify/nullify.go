...
func Fill(x interface{}) interface{} { // nolint:gocognit,cyclop // generated code
	coinType := reflect.TypeOf(sdk.Coin{})
	coinsType := reflect.TypeOf(sdk.Coins{})

	v := reflect.Indirect(reflect.ValueOf(x))
	switch v.Kind() { // nolint:exhaustive // test utils
	case reflect.Slice:
		for i := 0; i < v.Len(); i++ {
			obj := v.Index(i)
	...
	case reflect.Struct:
		for i := 0; i < v.NumField(); i++ {
			...
			switch f.Kind() { // nolint:exhaustive // test util
			case reflect.Slice:
